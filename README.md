...
- 特に**チームで協力して課題解決し合える環境**が肌に合っていると感じます。が、自律的にプロジェクトを推進していくような環境にも積極的に挑戦できます。

- 経験の積み重ねを大事にしており、何事もまずは挑戦してみる姿勢を大事にしています。

- 大きな課題に対しては、必要なタスクを細かく分解していき、順序よく解決していくことが得意です。

## 興味・関心

- **Android開発の領域が最も得意**で、最近はKotlin Multiplatformに興味があり、今後の発展に注目しています。
  - 他に、最近はレガシープロジェクトの最適なリファクタリング方法について考える事も多いです。

- 詳細設計~開発・テストフェーズの経験が多いですが、要件定義や内部設計フェーズにも興味があります。
...
- プロダクトの成長を肌で感じやすいBtoC向け事業に興味があり、ユーザーの視点に立ちやすい環境で活躍することを目指しています。

- モバイルアプリ開発を中心に、時代や状況に合わせて柔軟に立ち回れるエンジニアを目指しています。
  - あらゆる課題に対してポジション問わず挑戦していける環境だと望ましいです。

- 「一緒に仕事がしたい」「この人だったらお願いしたい」と思われるような信頼できる人物像を目指しています。
  - **チームワークとコミュニケーション**は特に重要視しており、技術の追求だけでなく、お互いに接しやすい距離感や気兼ねなく意見や相談ができる環境作りを意識しています。
...
| 分野           | 技術・ツール                             | 経験年数の目安 |
|----------------|------------------------------------------|--------------------------|
| モバイル開発   | Kotlin, Java, Flutter, Swift, Objective-C | 約5年〜6年               |
| Webフロント    | JavaScript, Vue.js                        | 約1年〜2年               |
| バックエンド   | PHP, Laravel                              | 約1年〜2年                    |
| DB             | MySQL, PostgreSQL, SQLite                 | 約2〜3年                 |
| インフラ/CI    | Docker, Firebase, GitHub, GitLab, Bitbucket | 約3年                    |
| テスト/CI      | Appium, OpenCV, Jenkins, JUnit, XCTest   | 約2年                    |
| その他         | JIRA, Confluence, Office系ツール         | 常用レベル               |
...
- 本プロダクト立ち上げ前まで着手していたプロダクトでは、「アーキテクチャの欠陥」による開発のしづらさが目立っており、今回の開発では改善したいと私が提案をしました。
  ...
  - 結果、開発効率は50%改善でき、どこにどう実装すればいいかがより明瞭になりました。また、モジュール単位でコードレビューできるようになり、レビュー時の負担は約50%以上改善できました。

  - 技術選定を担当したのはこれが初であり、技術面での設計スキルが磨かれました。

- チーム内でのコミュニケーションが非常に活発だったこともあり、特にコミュニケーションスキルは強く磨かれました。
...
- 技術的負債が原因で発生した幾つかの既存バグによってリリースできない状態だった為、1週間以内にリリース可能な状態となるよう改修する必要がありました。
  ...
  - 計画性と連携意識を持って取り組めたことで、予定より2日早くリリース可能な状態に回復することができました。この経験から、限られた期間で課題を解決するようなスケジュール管理能力が成長したと感じました。
...
- モバイルアプリとWebアプリ間の連携が必要な機能の開発を一人称で対応する必要がありました。

  - モバイルアプリからデータをサーバーへ送信し、送信されたデータをWebアプリで閲覧できることがゴールです。
  - 実現のため、アプリでは生成データをローカルDBへ一時保存し、WorkManagerを使用してデータ送信APIを定期実行するシステムを構築しました。
  - Webアプリでは、データを効率よく表示する為、ページネーションを用いたデータ取得・表示ロジックを構築することで、素早く閲覧できる機能を実装しました。
...
- フロントエンドとバックエンドの実装を同時に担当することが初めてだった為、私自身の技術的なインプット不足が課題でした。
  ...
  - 課題に対する取り組みとして、メンバーと1対1でコードレビューを実施し、実装コードを読み合うことで相互理解を深めながら開発を進めました。
...
- これまで手動でテストを実施していましたが、テストに大幅な工数がかかり、プロジェクト内でも大きな負荷になっていることが課題でした。
  ...
  - 本プロジェクトではAppiumを導入し、テスト自動化及びUI差分検出の仕組みを構築しました。
...
- テスト仕様書のメンテナンス性が悪く、テスト項目修正や実施に工数がかかっている状態でした。
...
- 当時は私がFlutterもPHPも業務未経験だった為、自身がキャッチアップをしながら開発業務をこなすことが課題でした。
  ...
  - また、コードレビューする側になったのも初めてであり、コードに対する読解力が磨かれた他、可読性の高いコーディングをより意識できるようになりました。
